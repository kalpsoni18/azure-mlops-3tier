...
output "keyvault" { value = module.keyvault.vault_name }
...
output "grafana_password_cmd" {
  value       = "az keyvault secret show --vault-name ${module.keyvault.vault_name} --name grafana-admin-password --query value -o tsv"
